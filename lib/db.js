...
export const DEFAULT_CARDS = [
  { step_number: 1, shape: "terminal", title: "1. TAHAP ADMINISTRASI & BERKAS PERSYARATAN", description: "Mahasiswa melengkapi berkas administrasi, verifikasi Link & Match, serta upload format PDF di GDRIVE MENTORA.", note: "Pengisian SKPI & Link & Match dapat dicicil sejak Semester 1 melalui Web Link & Match dan MENTORA (kpta.sisfoftudinus.my.id).", te_term: "Syarat Berkas (CD)", tind_term: "Syarat Berkas (PT)", tb_term: "Syarat Berkas (CD)", skip_tind: false, docs_json: JSON.stringify(DEFAULT_DOCS) },
  { step_number: 2, shape: "process",  title: "2. UJIAN PROPOSAL", description: "Mendaftar ujian proposal pada portal MENTORA dengan melampirkan berkas & Bukti Registrasi Link & Match.", note: "", te_term: "Capstone Design 1", tind_term: "Project Terpadu 1", tb_term: "Capstone Design 1 (Sempro)", skip_tind: false, docs_json: "[]" },
  { step_number: 3, shape: "note",     title: "UPDATE SKPI RUTIN & VERIFIKASI LINK & MATCH", description: "Memperbarui berkas SKPI secara rutin di Link & Match serta verifikasi di MENTORA.", note: "Rutin Smt 1 - Yudisium", te_term: "Rutin Smt 1 - Yudisium", tind_term: "Rutin Smt 1 - Yudisium", tb_term: "Rutin Smt 1 - Yudisium", skip_tind: false, docs_json: "[]" },
  { step_number: 4, shape: "decision", title: "3. UJIAN SEMINAR HASIL (SEMHAS)", description: "Penyampaian progres dan naskah publikasi ilmiah pada portal MENTORA sebelum mendaftar Sidang Akhir.", note: "", te_term: "Seminar Hasil (Semhas)", tind_term: "- (Langsung Sidang)", tb_term: "Seminar Hasil (Semhas)", skip_tind: true, docs_json: JSON.stringify(SEMHAS_DOCS) },
  { step_number: 5, shape: "process",  title: "4. SIDANG AKHIR", description: "Ujian pertanggungjawaban karya akhir Capstone Design 2 / Project Terpadu 2 di hadapan dewan penguji.", note: "Batas Akhir Validasi SKPI (Link & Match) wajib tervalidasi sebelum Yudisium.", te_term: "Capstone Design 2 (Sidang Akhir)", tind_term: "Project Terpadu 2 (Sidang Akhir)", tb_term: "Capstone Design 2 (Sidang Akhir)", skip_tind: false, docs_json: JSON.stringify(DEFAULT_DOCS) },
  { step_number: 6, shape: "process",  title: "5. YUDISIUM", description: "Penetapan kelulusan resmi setelah nilai sidang dan SKPI Link & Match tervalidasi penuh.", note: "Syarat SKPI (Link & Match) & MENTORA wajib tervalidasi penuh.", te_term: "Yudisium CD", tind_term: "Yudisium PT", tb_term: "Yudisium CD", skip_tind: false, docs_json: "[]" },
...
export async function initDb() {
  ...
  // Perjelas istilah Semhas & Capstone Design 2 (Sidang Akhir) agar tidak ambigu
  ...
  // Sinkronkan penyebutan MENTORA (Akademik & Ujian) dan Link & Match (SKPI & Portofolio)
  await pool.query(`UPDATE flow_cards SET description = 'Mahasiswa melengkapi berkas administrasi, verifikasi Link & Match, serta upload format PDF di GDRIVE MENTORA.', note = 'Pengisian SKPI & Link & Match dapat dicicil sejak Semester 1 melalui Web Link & Match dan MENTORA (kpta.sisfoftudinus.my.id).' WHERE step_number=1`);
  await pool.query(`UPDATE flow_cards SET description = 'Mendaftar ujian proposal pada portal MENTORA dengan melampirkan berkas & Bukti Registrasi Link & Match.' WHERE step_number=2`);
  await pool.query(`UPDATE flow_cards SET title = 'UPDATE SKPI RUTIN & VERIFIKASI LINK & MATCH', description = 'Memperbarui berkas SKPI secara rutin di Link & Match serta verifikasi di MENTORA.' WHERE step_number=3`);
  await pool.query(`UPDATE flow_cards SET description = 'Penyampaian progres dan naskah publikasi ilmiah pada portal MENTORA sebelum mendaftar Sidang Akhir.' WHERE step_number=4`);
  await pool.query(`UPDATE flow_cards SET note = 'Batas Akhir Validasi SKPI (Link & Match) wajib tervalidasi sebelum Yudisium.' WHERE step_number=5`);
